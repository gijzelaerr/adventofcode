...
def f(p, inc=10):
    ...
    for u, v in combinations(list(zip(*where(a == '#'))), 2):
        yield abs(u[0] - v[0]) + abs(u[1] - v[1])
        yield sum([inc - 1 for big in r if min(u[0], v[0]) < big < max(u[0], v[0])])
        yield sum([inc - 1 for big in c if min(u[1], v[1]) < big < max(u[1], v[1])])


print(sum(f('test.txt', inc=10)))  # should be 1030
print(sum(f('test.txt', inc=100)))  # should be 8410
print(sum(f('input.txt', inc=1000000)))
