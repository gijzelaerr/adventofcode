...
def find_full_number(lines, y: int, x: int) -> int:
...
def get_top_or_bottom(lines: List[str], y: int, x: int, check_top):
    if check_top:
        y -= 1
    else:
        y += 1
...
def get_sum_of_neighbors(lines: List[str], y: int, x: int):
